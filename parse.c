...
enum parse_oper_enum parse_oper_lookup(enum scan_token_enum tkid) {
    for (int i = 0; parse_oper_map[i].tkid != TK_NONE; i++) {
...
struct parse_node_st * parse_expression(struct parse_table_st *pt, 
    ...
    while (true) {
        ...
        enum parse_oper_enum oper = parse_oper_lookup(tp->id);
...
struct parse_node_st * parse_operand(struct parse_table_st *pt,
    ...
    if (scan_table_accept(st, TK_MINUS)) {
        ...
        np1 = parse_node_new(pt);
        np1->type = EX_INTVAL;
        uint32_t value = str_to_uint32(tp->value, 2);
        ...
        uint32_t value = str_to_uint32(tp->value, 10);
        np1->intval.value = value;
    } else if (scan_table_accept(st, TK_HEXLIT)) {
        tp = scan_table_get(st, -1);
        np1 = parse_node_new(pt);
        np1->type = EX_INTVAL;
        uint32_t value = str_to_uint32(tp->value, 16);
        np1->intval.value = value;
    } else if (scan_table_accept(st, TK_REGNAME)) {
    	tp = scan_table_get(st, -1);
    	np1 = parse_node_new(pt);
    	np1->type = EX_REGVAL;
    	np1->regname.name = tp->value;
        ...
        parse_error("Bad operand from parse");
    }
...
uint32_t str_to_uint32(char* input, int base){
	...
	while(input[i] != '\0'){
		if(base == 16 && input[i] >= 'A' && input[i] <= 'F'){
			temp = input[i] - 'A' + 10; 
		} else {
			temp = input[i] - '0'; 
		}
		if(value > max_val / base){
			printf("overflows uint32_t: %s",input);
			exit(-1);
		}
		value *= base;
		if (value > max_val - temp) {
		  printf("overflows uint32_t: %s",input);
		  exit(-1);
		}
		value += temp;
...
void parse_tree_print_expr(struct parse_node_st *np, int level) {
	...
	if (np->type == EX_REGVAL) {
		printf("REGVAL %s\n", np->regname.name);
	} else if (np->type == EX_INTVAL) {
        printf("INTVAL %d\n", np->intval.value);
